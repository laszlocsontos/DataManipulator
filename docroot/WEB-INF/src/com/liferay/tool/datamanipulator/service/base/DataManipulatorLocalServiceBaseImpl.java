...
public abstract class DataManipulatorLocalServiceBaseImpl
	implements DataManipulatorLocalService, IdentifiableBean {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	...
	public DataManipulator addDataManipulator(DataManipulator dataManipulator)
		...
		dataManipulator = dataManipulatorPersistence.update(dataManipulator,
				false);
		...
		if (indexer != null) {
			try {
				indexer.reindex(dataManipulator);
			}
	...
	public void deleteDataManipulator(long id)
		...
		DataManipulator dataManipulator = dataManipulatorPersistence.remove(id);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.delete(dataManipulator);
			}
			catch (SearchException se) {
				if (_log.isWarnEnabled()) {
					_log.warn(se, se);
	...
	private static Log _log = LogFactoryUtil.getLog(DataManipulatorLocalServiceBaseImpl.class);
